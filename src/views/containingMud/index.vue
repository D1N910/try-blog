        ...
        if (res.statusCode === 200) {
          ...
          res.content.forEach(element => {
            ...
            for (let i of matchEmojiWords) {
              ...
              const emojis18ListItem = emojis18List[i]
              if (emojiListItem) {
                element.content = element.content.replace(`[${i}]`, `<img src="${emojiListItem}" alt="[${i}]" class="emoji"/>`)
              } else if (emojis18ListItem) {
                element.content = element.content.replace(`[${i}]`, `<img src="${emojiListItem}" alt="[${i}]" class="emoji emoji_18"/>`)
              }
            }
